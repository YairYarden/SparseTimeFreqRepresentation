...
%% General Parameters
isRegularSt = true;
isHammingWindow = true;
isConstVarChirp = true;
...
if(isConstVarChirp)
    [constVarChirpSpecSlim, timeSpecSlim, freqSpecSlim] = ComputeSpecBySparseAlgo(inputSignal, timeVec, numIterations,...
                                                    fs, numSamplesInFrame, stepSize, numFreqBins,...
                                                    q, chirpWin, 'SLIM');
    %% Plot
    figure,
...
%     adaptiveVar = transpose(1./ (2*pi * instFreqVec) );
    ...
    subplot(2,2,3);
    surf(timeSpecSlim, freqSpecSlim, pow2db(constVarChirpSpecSlim), 'EdgeColor', 'none');
    axis xy; axis tight; colormap(jet); view(0,90);
    xlabel('Time'); colorbar; ylabel('Frequency(HZ)'); ylim([0,max(freqSpecSlim)]);
    title('ST-SLIM Spectrogram. Const Variance'); xlabel('Time[sec]'); ylabel('Freq[Hz]');    
    
    subplot(2,2,4);
    surf(timeSpecSlim, freqSpecSlim, pow2db(chirpSpecSlim), 'EdgeColor', 'none');
    axis xy; axis tight; colormap(jet); view(0,90);
    xlabel('Time'); colorbar; ylabel('Frequency(HZ)'); ylim([0,max(freqSpecSlim)]);
    title('ST-SLIM Spectrogram. adaptive variance. zeta = 30 * fs'); xlabel('Time[sec]'); ylabel('Freq[Hz]');
end
